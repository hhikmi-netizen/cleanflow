...
export default async function TrackingPage({ params }: { params: { token: string } }) {
  // Use anon key — only non-sensitive fields exposed
  ...
  const { data: order } = await supabase
    .from('orders')
    .select('id, order_number, status, total, deposit, paid, created_at, pickup_date, notes, order_items(service_name, quantity, subtotal, article_code), pressings(name, phone, address), clients(name, phone)')
  ...
  const totalPaid = Number(order.deposit || 0) + (payments || []).reduce((s, p) => s + Number(p.amount), 0)
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-md mx-auto space-y-4">
        ...
        <div className="text-center mb-6">
          ...
          {pressing?.phone && (
            <a href={`tel:${pressing.phone}`} className="text-sm text-blue-500 flex items-center justify-center gap-1 mt-1">
                ...
                <div className="flex justify-between">
                  {STATUS_STEPS.map((s, i) => (
                    <div key={s} className="flex flex-col items-center flex-1">
                      ...
                      <p className={`text-xs mt-1 text-center ${i <= statusIndex ? 'text-blue-600 font-medium' : 'text-gray-300'}`}>
                        {s === 'pending' ? 'Reçu' : s === 'in_progress' ? 'En traitement' : s === 'ready' ? 'Prêt' : 'Livré'}
                      </p>
                    </div>
                  ))}
                </div>
        ...
        {pressing?.phone && (
          <a
            href={buildWhatsApp(pressing.phone, whatsappMsg)}
            target="_blank"
        ...
        <p className="text-center text-xs text-gray-300">CleanFlow · Suivi commande</p>
      </div>
    </div>
  )
}
